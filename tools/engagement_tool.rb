...
class EngagementTool
   ...
   def set_account_config(file)
     ...
     @client.set_keys keys['engagement_api']
	  rescue
...
=end
   ...
   def format_request_settings
    settings = {}
    settings['engagement_settings'] = {}
    settings['engagement_settings']['endpoint'] = @endpoint
	  ...
	  if @endpoint == 'historical'
		 ...
		 settings['engagement_settings']['start'] = @utils.get_ISO_date_string(@utils.get_date_object(@start_date)) unless @start_date.nil?
		 settings['engagement_settings']['end'] = @utils.get_ISO_date_string(@utils.get_date_object(@end_date)) unless @end_date.nil? #Let API default to now.
	  end

	  settings['engagement_types'] = []

	  if @endpoint == 'totals'
		 @engagement_types.each do |engagement_type|
			if TOTALS_ENGAGEMENT_TYPES.include?(engagement_type[0])
			   if engagement_type[1] then
				  settings['engagement_types'] << engagement_type[0]
			   end
			end
		 end
	  else
    ...
    settings['engagement_groupings'] = {}

	  @groupings.each do |key, items|
		  if @endpoint == 'totals' and (!items.include? 'engagement.hour' or !items.include? 'engagement.day')
        AppLogger.log_info "Not adding time-series grouping to /totals settings"
        @groupings = @groupings.tap { |h| h.delete(key) }
      else
        settings['engagement_groupings'][key] = items
	    end
    end
   ...
   def manage_process(endpoint = nil)
    ...
    request_settings = format_request_settings
    @client.set_settings request_settings
	  ...
	  @tweets_of_interest.each do |toi| #These tweets_of_interest items each map to a single request.
		 ...
		 begin
			...
			results['request'] = @client.create_request(toi)
			start_request = Time.now
			@num_requests += 1
			AppLogger.log_info "Making #{@num_requests} of #{@tweets_of_interest.count} requests..."
			results['response'] = @client.make_request.body
			...
			AppLogger.log_error "ERROR occurred, skipping request. #{e.message}"
		 end
	  end
